...
unsafe fn transpose_block<T: Copy>(input: &[T], output: *mut T, width: usize, height: usize, start_x: usize, start_y: usize, block_width: usize, block_height: usize) {
    for inner_x in 0..block_width {
        for inner_y in 0..block_height {
            ...
            unsafe {
                *output.add(output_index) = *input.get_unchecked(input_index);
            }
        }
    }
}
...
// TODO: Perhaps delete this? Irrelevant for large NTTs
/// Given an array of size width * height, representing a flattened 2D array,
/// transpose the rows and columns of that 2D array into the output.
...
///
/// EDIT: `output` is the pointer cast to usize (Send+Sync, like derange)
///the two recursive halves cover independant output regions
fn transpose_recursive<T: Copy + Send + Sync>(input: &[T], output: usize, row_start: usize, row_end: usize, col_start: usize, col_end:  usize, total_columns: usize, total_rows: usize) {
...
pub fn transpose_par<T: Copy + Send + Sync>(input: &[T], output: &mut [T], input_width: usize, input_height: usize) {
    ...
    if input.len() <= SMALL_LEN {
        ...
        // EDIT: cast output ptr to usize (Send+Sync) bound
        let output = output.as_mut_ptr() as usize;
